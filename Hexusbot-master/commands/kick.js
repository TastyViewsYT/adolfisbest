exports.run = async function(bot, msg, args) {
  ...
  if (!user) return bot.embed(msg, bot.hex, "Invalid user mention:", "Couldn't find mention or user is invalid.");
  ...
  bot.fetchMember(msg, user).then(function(member) {
    ...
    bot.embed(msg, bot.hex, "Successfully kicked user:", `User: ${user.tag} \nReason: ${reason}`);
