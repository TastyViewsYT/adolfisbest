...
exports.run = function(bot, msg, args) {
  ...
  if (user) msg.guild.fetchMember(user).then(function(member) {
    if (!blacklist[member.id]) blacklist[member.id] = {blacklist: 1};
    else blacklist[member.id] = {blacklist: 1}
    bot.embed(msg, bot.hex, "Successfully blacklisted user:", `• Blacklisted: ${member.user.tag} \nThey will no longer be able to use the music commands.`);
...
exports.help = {
  name: 'blacklist',
  description: 'blacklists the mentioned user from music commands.',
  usage: 'blacklist'
};